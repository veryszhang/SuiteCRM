...
class InboundEmail extends SugarBean
{
    ...
    public function markEmails($uids, $type)
    {
        switch ($type) {
            case 'unread':
                imap_clearflag_full($this->conn, $uids, '\\SEEN', ST_UID);
                break;
            case 'read':
                imap_setflag_full($this->conn, $uids, '\\SEEN', ST_UID);
                break;
            case 'flagged':
                imap_setflag_full($this->conn, $uids, '\\FLAGGED', ST_UID);
                break;
            case 'unflagged':
                imap_clearflag_full($this->conn, $uids, '\\FLAGGED', ST_UID);
                break;
            case 'answered':
                imap_setflag_full($this->conn, $uids, '\\Answered', ST_UID);
                break;
        }
    }
     ...
     * @param string $userName  Name of account, for Sugar purposes
     ...
     *
     * @return string|bool returns GUID of Group User if user_name match is found, false if NO DUPE IS FOUND
     */
     ...
     *
     * @param $email Email passed as reference
     * @param $contactAddr
     */
     ...
     *
     * @return array decodedHeader associative array
     */
     ...
     *
     * @param array  $parts      array of objects
     * @param string $subtype    what type of trail to return? HTML? Plain? binaries?
     * @param string $breadcrumb text trail to build up
     *
     * @return string
     */
     ...
     * @param int|string $breadcrumb         Passed integer value to start breadcrumb trail
     ...
     *
     * @param array $arr an array of email address objects
     *
     * @return string
     */
     ...
     * @param int    $enc detected encoding
     *
     * @return string
     */
     ...
     */
    public function getDuplicateEmailId($msgNo)
    {
     ...
     *
     * @param string $emailName the subject of the email to match
     * @param string $tableName the table of the matching bean type
     *
     * @return string|bool
     */
...
eoq;
     ...
     *
     * @return string|bool Case ID or FALSE if not found
     */
     ...
     *
     * @return resource|bool Connection resource on success, FALSE on failure
     */
     ...
     *
     * @param int $msgNo messageNo of a message
     *
     * @return string UIDL for the message
     */
...
eoq;
     ...
     */
    public function sortFetchedOverview($arr, $sort = 4, $direction = 'DESC')
    {
        global $current_user;
    ...
    public function displayFolderContents($mbox, $forceRefresh = 'false', $page)
    {
        ...
        $ret = array();
        $cacheUsed = false;
        if ($forceRefresh == 'false' && $this->validCacheExists()) {
            $emailSettings = $current_user->getPreference('emailSettings', 'Emails');
        ...
        $out = $this->displayFetchedSortedListXML($ret);
    ...
    public function displayFetchedSortedListXML($ret)
    {
        global $timedate;
        global $sugar_config;
